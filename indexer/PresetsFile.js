...
class PresetsFile {
	constructor(baseDir, relativeFilePath, settings, errors) {
		...
		if (undefined === this.priority) {
			this.priority =
				this._settings.PresetCategoriesPriorities[this.category];
		}
	...
	_checkProperties() {
		...
		if (undefined !== this._currentOption) {
			this._addError(
				`Missing ${this._settings.OptionsDirectives.END_OPTION_DIRECTIVE} for ${this._currentOption.name}`
			);
		}

		if (undefined !== this._currentOptionGroup) {
			this._addError(
				`Missing ${this._settings.OptionsDirectives.END_OPTION_GROUP_DIRECTIVE} for ${this._currentOptionGroup.name}`
			);
		}
	}
	...
	_processMetapropertyLine(line) {
		...
		if (
			!isProperty &&
			lowCaseLine.startsWith(
				this._settings.OptionsDirectives.OPTION_DIRECTIVE
			)
		) {
		...
		if (!isProperty && !isOptionDirective) {
			if (isPropertyMissingSemicolon) {
				this._addError(
					`line ${this._currentLine}, property missing ":"`
				);
			} else {
				this._addError(
					`line ${this._currentLine}, unknown preset directive: '${line}'`
				);
			}
		}
	}
	...
	_processOptionDirective(line) {
		...
		if (
			lowCaseLine.startsWith(
				this._settings.OptionsDirectives.BEGIN_OPTION_DIRECTIVE
			)
		) {
			this._processOptionBeginDirective(line, lowCaseLine);
		} else if (
			lowCaseLine.startsWith(
				this._settings.OptionsDirectives.END_OPTION_DIRECTIVE
			)
		) {
			this._processOptionEndDirective(line, lowCaseLine);
		} else if (
			lowCaseLine.startsWith(
				this._settings.OptionsDirectives.BEGIN_OPTION_GROUP_DIRECTIVE
			)
		) {
			this._processOptionGroupBeginDirective(line, lowCaseLine);
		} else if (
			lowCaseLine.startsWith(
				this._settings.OptionsDirectives.END_OPTION_GROUP_DIRECTIVE
			)
		) {
	...
	_processOptionGroupBeginDirective(line, lowCaseLine) {
		...
		if ("" === optionGroup.name) {
			...
			this._addError(
				`line ${this._currentLine}, nested #$ option groups are not allowed`
			);
		} else {
	...
	_processOptionGroupEndDirective(line, lowCaseLine) {
		if (undefined === this._currentOptionGroup) {
			this._addError(
				`line ${this._currentLine}, end Option Group directive found but no Option Group to close`
			);
		} else {
			const lowCaseOptionGroupName =
				this._currentOptionGroup.name.toLowerCase();

			const indexOfOption = this.optionGroups.findIndex(
				(item) => lowCaseOptionGroupName === item.name.toLowerCase()
			);
	...
	_processOptionBeginDirective(line, lowCaseLine) {
		...
		if ("" === Option.name) {
			...
			this._addError(
				`line ${this._currentLine}, nested #$ options are not allowed`
			);
		} else {
	...
	_processOptionEndDirective(line, lowCaseLine) {
		if (undefined === this._currentOption) {
			this._addError(
				`line ${this._currentLine}, end Option directive found but no Option to close`
			);
		} else {
			const lowCaseOptionName = this._currentOption.name.toLowerCase();

			const indexOfOption = this.options.findIndex(
				(item) => lowCaseOptionName === item.name.toLowerCase()
			);
	...
	_getOptionGroup(line) {
		...
		if (0 == directiveRemoved.length || directiveRemoved[0] != ":") {
			this._addError(
				`line ${this._currentLine}, OPTION_GROUP BEGIN directive should be followed by ":". Example: #$ OPTION_GROUP BEGIN: My Group Name`
			);
		}
	...
	_getOption(line) {
		const directiveRemoved = line
			.slice(
				this._settings.OptionsDirectives.BEGIN_OPTION_DIRECTIVE.length
			)
			.trim();
		...
		const regExpRemoveUnchecked = new RegExp(
			this._escapeRegex(
				this._settings.OptionsDirectives.OPTION_UNCHECKED
			),
			"gi"
		);
		...
		if (0 == optionName.length || optionName[0] != ":") {
			this._addError(
				`line ${this._currentLine}, OPTION BEGIN directive should be followed by ":". Example: #$ OPTION BEGIN (UNCHECKED): My Option Name`
			);
		}
	...
	_isOptionChecked(lowCaseLine) {
		...
		if (
			lowCaseLine.includes(
				this._settings.OptionsDirectives.OPTION_CHECKED
			)
		) {
			OptionChecked = true;
		}
		...
		if (OptionChecked && OptionUnchecked) {
			this._addError(
				`line ${this._currentLine}, Option can't be checked and unchecked at the same time`
			);
		} else if (!OptionChecked && !OptionUnchecked) {
			this._addError(
				`line ${
					this._currentLine
				}, Every option must specify whether it is ${this._settings.OptionsDirectives.OPTION_CHECKED.toUpperCase()} or ${this._settings.OptionsDirectives.OPTION_UNCHECKED.toUpperCase()}`
			);
		} else {
	...
	_processProperty(property, line) {
		switch (this._presetsFileMetadata[property].type) {
			...
			default:
				this._addError(
					`line ${this._currentLine}, unknown property type '${this._presetsFileMetadata[property].type}' for the property '${property}'`
				);
		}
	}
	...
	_processPresetStatusProperty(property, line) {
		...
		if (this._settings.PresetStatusEnum.includes(line)) {
			...
			this._addError(
				`line ${this._currentLine}, unknown ${property} values: '${line}'; available values: ${this._settings.PresetStatusEnum}`
			);
		}
	}
	...
	_processBooleanProperty(property, line) {
		...
		if (trueValues.includes(lineLowCase)) {
			...
			this._addError(
				`line ${this._currentLine}, boolean property '${property}'' has a wrong value: '${line}'`
			);
		}
	...
	_checkPropertyDublicated(property) {
		if (undefined !== this[property]) {
			this._addError(
				`line ${this._currentLine}, duplicated property '${property}'`
			);
		}
	}
	...
	_processFilePathProperty(property, line) {
		...
		if (!stat || stat.isDirectory()) {
			this._addError(
				`line ${this._currentLine}, can't find file '${line}'`
			);
		} else {
			this[property] = line;
		}
	...
	_processFilePathArrayProperty(property, line) {
		...
		if (fs.existsSync(line)) {
			...
			if (!stat || stat.isDirectory()) {
				this._addError(
					`line ${this._currentLine}, a folder is specified instead of a file: '${line}'`
				);
			} else {
	...
	_processPresetCategoryProperty(property, line) {
		...
		for (const [key, value] of Object.entries(
			this._settings.PresetCategories
		)) {
		...
		if (!presetTypeValid) {
			this._addError(
				`line ${this._currentLine}, unknown preset category: '${line}'`
			);
		}
	}
	...
	_processPriorityProperty(property, line) {
		...
		if (NaN === value || value !== value2) {
			this._addError(
				`line ${this._currentLine}, PRIORITY value must be integer. Instead it is: '${line}'`
			);
		}
